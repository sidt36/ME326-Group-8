    ...
    tf_broadcaster(this)
{
    ...
    coordinate_frame_name = declare_parameter("coordinate_frame_name", "camera_frame", descr("coordinate frame name for camera", true));
                            ...
                            const sensor_msgs::msg::CameraInfo::ConstSharedPtr& msg_ci)
{
    ...
    for(int i = 0; i < zarray_size(detections); i++) {
        ...
        tf.header.frame_id = coordinate_frame_name;
        // Log the frame name
        // RCLCPP_INFO(get_logger(), "Frame name: %s", coordinate_frame_name.c_str());
